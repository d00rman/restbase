...
Router.prototype.handleResources = function(restbase) {
    ...
    return this.tree.visitAsync(function(value, path) {
        console.log("\n\nhandleRes(): VISIT ASYNC START FOR " + JSON.stringify(path) + "\n");
        if (value && Array.isArray(value.resources) && value.resources.length > 0) {
            return P.each(value.resources, function(reqSpec) {
                ...
                return restbase.request(req).then(function(ret) { console.log("\nrouter:handleRes(): REQ completed\n\n"); return ret; });
            }).then(function(ret) { console.log("\nrouter:handleRes(): P.each completed for " + path.toString()  + "\n\n"); return ret; });
        } else {
